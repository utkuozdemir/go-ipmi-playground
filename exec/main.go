...
func (c *Client) AttemptUserSetup(username, password string, logger *zap.Logger) error {
	...
	logger.Info("got user summary", zap.Uint8("max_users", summResp.MaxUsers))
	...
	for i := uint8(2); i <= maxUsers; i++ {
		logger.Info("check slot", zap.Uint8("num", i))

		userRes, userErr := c.getUserName(i)
		if userErr != nil {
			logger.Warn("got error", zap.Uint8("num", i), zap.Error(userErr))
		...
		if userRes.Username == username {
			logger.Info("user found", zap.String("username", username), zap.Uint8("id", userID))
